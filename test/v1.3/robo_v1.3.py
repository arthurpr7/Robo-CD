...
class IndividuoPG:
    ...
    def criar_arvore_aleatoria(self):
        ...
        # Adicionar operadores mais sofisticados
        ...
        elif operador in ['abs', 'sin', 'cos']:  # Adicionar sin e cos
            return {
                'tipo': 'operador',
                'operador': operador,
                'esquerda': IndividuoPG(self.profundidade - 1).arvore_aceleracao,
    ...
    def criar_folha(self):
        ...
        if tipo == 'constante':
            # Valores mais moderados para constantes
            if random.random() < 0.5:  # Para aceleração
                return {
                    'tipo': 'folha',
                    'valor': random.uniform(-1, 1)  # Limitado entre -1 e 1
                }
            else:  # Para rotação
                return {
                    'tipo': 'folha',
                    'valor': random.uniform(-0.5, 0.5)  # Limitado entre -0.5 e 0.5
                }
        else:
            return {
                'tipo': 'folha',
    ...
    def avaliar_no(self, no, sensores):
        ...
        # Adicionar operadores trigonométricos
        if no['operador'] == 'sin':
            valor = self.avaliar_no(no['esquerda'], sensores)
            return np.sin(min(max(valor, -100), 100))  # Limitar entrada para evitar NaN
        elif no['operador'] == 'cos':
            valor = self.avaliar_no(no['esquerda'], sensores)
            return np.cos(min(max(valor, -100), 100))  # Limitar entrada para evitar NaN
        elif no['operador'] == 'abs':
    ...
    def mutacao_no(self, no, probabilidade):
        if random.random() < probabilidade:
            if no['tipo'] == 'folha':
                if 'valor' in no:
                    if random.random() < 0.5:  # Para aceleração
                        no['valor'] = random.uniform(-1, 1)  # Limitado entre -1 e 1
                    else:  # Para rotação
                        no['valor'] = random.uniform(-0.5, 0.5)  # Limitado entre -0.5 e 0.5
                elif 'variavel' in no:
                ...
                no['operador'] = random.choice(['+', '-', '*', '/', 'max', 'min', 'abs', 'if_positivo', 'if_negativo', 'sin', 'cos'])
    ...
    def crossover_no(self, no1, no2):
        # Probabilidade de crossover ajustada para 0.7
...
class ProgramacaoGenetica:
    ...
    def avaliar_populacao(self):
        ...
        for individuo in self.populacao:
            ...
            for _ in range(5):
                ...
                # Calcular fitness
                fitness_tentativa = (
                    robo.recursos_coletados * 150 +  # Aumentado de 100 para 150
                    robo.distancia_percorrida * 0.1 -  # Mantido
                    robo.colisoes * 100 -  # Aumentado de 50 para 100
                    (100 - robo.energia) * 1.0  # Aumentado de 0.5 para 1.0
                )
                
                # Adicionar recompensa por proximidade à meta
                dist_meta = np.sqrt((robo.x - ambiente.meta['x'])**2 + (robo.y - ambiente.meta['y'])**2)
                fitness_tentativa += (ambiente.largura - dist_meta) * 0.2
                
                # Aumentar pontos extras por atingir a meta
                if robo.meta_atingida:
                    fitness_tentativa += 1000  # Aumentado de 500 para 1000
    ...
    def selecionar(self):
        # Aumentar o tamanho do torneio para 5
    ...
    def evoluir(self, n_geracoes=50):
        ...
        for geracao in range(n_geracoes):
            ...
            # Elitismo adaptativo - manter os melhores 5% indivíduos
            melhores = sorted(self.populacao, key=lambda x: x.fitness, reverse=True)
            num_elite = max(1, int(0.05 * self.tamanho_populacao))  # 5% da população, no mínimo 1
            nova_populacao.extend(melhores[:num_elite])
            
            # Preencher o resto da população
            while len(nova_populacao) < self.tamanho_populacao:
                pai1, pai2 = random.sample(selecionados, 2)
                filho = pai1.crossover(pai2)
                filho.mutacao(probabilidade=0.15)  # Aumentado de 0.1 para 0.15
