...
class IndividuoPG:
    ...
    def criar_arvore_aleatoria(self):
        ...
        elif operador in ['if_greater', 'if_less']:
            return {
                'tipo': 'operador',
                'operador': operador,
                'esquerda': IndividuoPG(self.profundidade - 1).arvore_aceleracao,
    ...
    def avaliar_no(self, no, sensores):
        ...
        if no['tipo'] == 'folha':
            if 'valor' in no:
                ...
                if no['variavel'] == 'meta_atingida':
                    return 1 if sensores[no['variavel']] else 0
        ...
        if no['operador'] == 'abs':
            ...
            return np.tanh(self.avaliar_no(no['esquerda'], sensores))
        elif no['operador'] == 'if_greater':
            esquerda = self.avaliar_no(no['esquerda'], sensores)
            direita = self.avaliar_no(no['direita'], sensores)
            return esquerda if esquerda > 0 else direita
        elif no['operador'] == 'if_less':
            esquerda = self.avaliar_no(no['esquerda'], sensores)
            direita = self.avaliar_no(no['direita'], sensores)
            return esquerda if esquerda < 0 else direita
        ...
        if no['operador'] == '+':
            ...
            return esquerda / direita if abs(direita) > 0.01 else esquerda  # Evitar divisão por zero
    ...
    def mutacao_no(self, no, probabilidade):
        if random.random() < probabilidade:
            if no['tipo'] == 'folha':
                ...
                no['operador'] = random.choice(['+', '-', '*', '/', 'max', 'min', 'abs', 'sin', 'cos', 'tanh', 'if_greater', 'if_less'])
    ...
    def crossover_no(self, no1, no2):
        ...
        if random.random() < 0.7:  # Aumentada para favorecer o crossover
            return no1.copy()
        else:
            return no2.copy()
...
class ProgramacaoGenetica:
    ...
    def avaliar_populacao(self):
        ...
        for individuo in self.populacao:
            ...
            for _ in range(5):  # Reduzido para 5 para acelerar a evolução
                ambiente.reset()
                x_inicial, y_inicial = ambiente.posicao_segura(robo.raio)  # Posição inicial segura
                ...
                # Calcular fitness
                fitness_tentativa = (
                    robo.recursos_coletados * 150 +  # Aumentado: Pontos por recursos coletados
                    robo.distancia_percorrida * 0.2 -  # Aumentado: Pontos por distância percorrida
                    robo.colisoes * 70 -  # Aumentado: Penalidade por colisões
                    (100 - robo.energia) * 0.5 -  # Penalidade por consumo de energia
                    robo.tempo_parado * 2  # Nova penalidade por ficar parado
                )
                
                # Adicionar pontos extras por atingir a meta
                if robo.meta_atingida:
                    fitness_tentativa += 1000  # Aumentado: Pontos extras por atingir a meta
                    fitness_tentativa += robo.energia * 5  # Bônus pela energia restante ao atingir a meta
                
                fitness += max(0, fitness_tentativa)
            
            individuo.fitness = fitness / 5  # Média das tentativas
    ...
    def selecionar(self):
        ...
        # Seleção por torneio
        tamanho_torneio = 5  # TAMANHO DO TORNEIO PARA O ALUNO MODIFICAR
        selecionados = []
    ...
    def evoluir(self, n_geracoes=30):
        ...
        for geracao in range(n_geracoes):
            ...
            print(f"Geração {geracao+1}/{n_geracoes}, Melhor fitness: {self.melhor_fitness:.2f}")
            ...
            nova_populacao.extend(ordenados[:2])
            
            # Preencher o resto da população
            while len(nova_populacao) < self.tamanho_populacao:
                pai1, pai2 = random.sample(selecionados, 2)
                filho = pai1.crossover(pai2)
                filho.mutacao(probabilidade=0.2)  # PROBABILIDADE DE MUTAÇÃO PARA O ALUNO MODIFICAR
